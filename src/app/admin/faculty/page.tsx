...
import { Plus, Pencil, UserCircle2, ExternalLink } from 'lucide-react';
...
export default async function AdminFaculty() {
  ...
  const { data: members } = await supabase
    .from('faculty')
    .select('id, name, designation, department, qualification, experience_years, photo_url, email, display_order, is_active, synced_from_api, staff_id, last_synced_at, status')
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl">
      ...
      {members && members.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {members.map((m) => {
            const isSynced = m.synced_from_api === true;
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-center gap-2 pt-2 border-t border-gray-50">
                  {isSynced ? (
                    <a
                      href={`https://www.jkkn.ai/admin/staff/${m.id}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1.5 text-xs font-medium text-blue-600 hover:text-blue-800 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition"
                    >
