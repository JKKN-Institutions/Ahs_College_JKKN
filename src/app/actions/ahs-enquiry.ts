// DEP-34 — server action behind the allied health sciences enquiry form.
//
...
function validate(name: string, phone: string, programme: string) {
  const fieldErrors: AhsEnquiryState["fieldErrors"] = {}
  ...
  if (!VALID_AHS_PROGRAMME_IDS.has(programme))
    fieldErrors.programme = "Please choose a programme."
  return fieldErrors
}

export async function submitAhsEnquiry(
  _prevState: AhsEnquiryState | null,
  formData: FormData
): Promise<AhsEnquiryState> {
  ...
  const payload = {
    ...
    utmSource: "ahs.jkkn.ac.in",
    utmMedium: "site-form",
    utmCampaign: sourcePage ? `ahs-${sourcePage}` : "ahs-site",
    campaignLinkId: null,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[ahs-enquiry] CRM rejected submit", res.status, body)
      return {
        success: false,
        error:
    ...
    console.error("[ahs-enquiry] network/timeout reaching CRM", err)
    return {
      success: false,
      error:
