...
import { buildCityFaqs, type CityPage } from "@/lib/city-pages";
...
export function CityPageContent({ city }: CityPageContentProps) {
  return (
    <>
      ...
      <Navbar />
      <Breadcrumb items={[{ label: city.cityName }]} />
      <main id="main-content">
        <TopBanner />
        <HeroSection city={city} />
        <AffiliationBar />
        <DistanceCard city={city} />
        <WhyChooseSection city={city} />
        <ProgrammesSection />
        <PlacementHighlights />
        <CompetitiveAdvantageSection city={city} />
        <HowToReachSection city={city} />
        <FacilitiesSection city={city} />
        <CampusGallerySection city={city} />
        <SnippetBaitSection city={city} />
        <CityFAQAccordion cityName={city.cityName} faqs={buildCityFaqs(city)} />
        <ExploreCitiesSection city={city} />
      </main>
      <Footer />
    </>
  );
...
/* ──────────────────────────────────────────
   HERO SECTION
────────────────────────────────────────── */
function HeroSection({ city }: { city: CityPage }) {
  return (
    <section className="bg-gradient-to-br from-[#006837] via-[#006837] to-[#002309] text-white py-16 md:py-24 relative overflow-hidden">
      <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a
            href="https://www.jkkn.ai/apply/jkkn-admission-2026?utm_source=ahs.jkkn.ac.in&utm_medium=organic&utm_campaign=components-citypagecontent"
            target="_blank"
...
/* ──────────────────────────────────────────
   DISTANCE CARD
────────────────────────────────────────── */
function DistanceCard({ city }: { city: CityPage }) {
  return (
      ...
      <div className="max-w-3xl mx-auto">
        <div className="bg-white rounded-2xl shadow-sm flex flex-col sm:flex-row items-center gap-4 p-6">
          <div className="flex-1 text-center sm:text-left">
            <span className="text-5xl font-black text-[#006837]">~{city.distanceKm}</span>
            <span className="text-xl font-bold text-[#006837] ml-1">km</span>
          </div>
          <div className="flex-[3] text-center sm:text-left">
            <div className="font-bold text-gray-800 text-base">
              From {city.cityName} to JKKN AHS
            </div>
            <div className="text-gray-500 text-sm mt-1">
              {city.travelTime} {city.routeVia}
            </div>
          </div>
          <a
...
/* ──────────────────────────────────────────
   WHY CHOOSE SECTION
────────────────────────────────────────── */
function WhyChooseSection({ city }: { city: CityPage }) {
  const reasons = [
    {
      icon: TrendingUp,
    ...
    },
    {
      ...
      desc: "International career opportunities across hospitals in the Gulf, UK, and Australia",
    },
    {
      icon: city.tier === "commute" ? Bus : Home,
      title: city.commuteNote.title,
      desc: city.commuteNote.desc,
    },
  ];
    ...
    <section className="bg-white py-14 px-4">
      <div className="max-w-5xl mx-auto text-center">
        <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-4">
          Why {city.cityName} Students Choose JKKN AHS
        </h2>
        ...
        <p className="text-center text-sm text-gray-500 mt-8">
          ...
          {city.nearbyCities.map((c, i) => (
            <span key={c.name}>
              <a href={c.href} className="text-[#006837] hover:underline font-medium">
                {c.name} ({c.distance})
              </a>
              {i < city.nearbyCities.length - 1 ? ", " : "."}
            </span>
          ))}
        </p>
      </div>
...
────────────────────────────────────────── */
function CompetitiveAdvantageSection({ city }: { city: CityPage }) {
  const advantages = [
    {
    ...
    {
      title: "NAAC Accredited & University Affiliated",
      desc: "Accredited by NAAC and affiliated to The Tamil Nadu Dr. M.G.R. Medical University, Chennai — your degree is recognized nationwide and internationally.",
    },
  ];
...
────────────────────────────────────────── */
function HowToReachSection({ city }: { city: CityPage }) {
  const routes = [
    {
      icon: Map,
      label: "ROUTE",
      desc: `${city.routeVia.charAt(0).toUpperCase()}${city.routeVia.slice(1)} — campus is on NH-544 (Salem–Coimbatore Highway) at Komarapalayam`,
    },
    {
      icon: Bus,
      label: "BY BUS",
  ...
  return (
    <section className="bg-[#FBFBEE] py-14 px-4">
      <div className="max-w-3xl mx-auto text-center">
        <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-3">
          How to Reach from {city.cityName}
        </h2>
        <p className="text-gray-500 text-sm mb-3">
        ...
        <div className="bg-[#006837] text-white rounded-t-2xl px-6 py-4 flex items-center gap-3 text-left">
          ...
          <div>
            <div className="font-bold text-base">
              {city.cityName} → JKKN AHS Campus
            </div>
            <div className="text-white/70 text-xs mt-0.5">
              ~{city.distanceKm} km • {city.travelTime}
            </div>
          </div>
        </div>
        ...
        <p className="text-xs text-gray-400 mt-4 text-left">
          Also convenient for students from {city.nearbyAreas.join(", ")}.
        </p>
      </div>
    </section>
  );
...
/* ──────────────────────────────────────────
   FACILITIES SECTION
────────────────────────────────────────── */
function FacilitiesSection({ city }: { city: CityPage }) {
  const facilities = [
    {
      icon: Microscope,
      title: "Modern Labs",
    ...
    },
    {
      icon: Bus,
      title: "Transport",
      desc: `College buses connecting to ${city.cityName} side and surrounding areas`,
    },
    {
      icon: Trophy,
      title: "Sports",
...
────────────────────────────────────────── */
function SnippetBaitSection({ city }: { city: CityPage }) {
  return (
    <section className="bg-[#FBFBEE] py-14 px-4">
      <div className="max-w-3xl mx-auto">
        <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-3 text-center">
          Allied Health Sciences in {city.cityName} — Quick Guide
        </h2>
        <div className="w-12 h-1 bg-[#7cb983] rounded mx-auto mb-10"></div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <h3 className="font-bold text-gray-900 mb-3">Top Allied Health Science Courses Available Near {city.cityName}</h3>
...
────────────────────────────────────────── */
function ExploreCitiesSection({ city }: { city: CityPage }) {
  return (
    <section className="bg-[#FBFBEE] py-14 px-4">
      <div className="max-w-3xl mx-auto text-center">
        <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-3">
